...
if ($ScriptDir -and (Test-Path (Join-Path $ScriptDir "..\bin\multi-desktop-notify.py"))) {
...
if ($ScriptDir -and (Test-Path (Join-Path $ScriptDir "scripts\install.ps1"))) {
    ...
    & powershell -ExecutionPolicy Bypass -File (Join-Path $ScriptDir "scripts\install.ps1")
    exit 0
} elseif ($ScriptDir -and (Test-Path (Join-Path $ScriptDir "install.ps1"))) {
    Write-Host "=== Cap nhat tu thu muc ma nguon cuc bo ===" -ForegroundColor Cyan
...
try {
    ...
    $SourceDir = Join-Path $TempZipDir "agent-notifier-master"
    $InstallScript = Join-Path $SourceDir "scripts\install.ps1"
