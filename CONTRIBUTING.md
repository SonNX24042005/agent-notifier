...
3. **Synchronize both platforms for installer and configuration changes**:
   - Any modification affecting CLI contracts, hook configurations, or installed artifacts must be implemented in both Linux scripts (`install.sh`, `update.sh`, `uninstall.sh`) and Windows scripts (`install.ps1`, `update.ps1`, `uninstall.ps1`).
